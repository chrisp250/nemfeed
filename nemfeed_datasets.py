"""Module for downloading data different 'CURRENT' datasets
(the selected data files reside in http://www.nemweb.com.au/Reports/CURRENT)
...
import requests
from nemfeed import nemfile_reader


class CurrentFileHandler:
    """class for handling 'CURRENT' nem reports from http://www.nemweb.com.au
    ...
    def __init__(self):
        ...
        self.filetime_max = 0
        self.foundit=False #Is there a new file?

    #Did the last update return a new file
    def isNewfile():
        return self.foundit
        #return datafeed
    def getDatafeed():
    ...
    def update_data(
        ...
        for match in regex.finditer(page.text):
            ...
            if (int(match.group(1)) > self.filetime_max):
                max_name=match.group(0)
                self.foundit=True
...
#                    print(table,dataframe)
        if (self.foundit):
            self.filetime_max=int(match.group(1))
            print("Found:"+max_name)
            self.nemfile = self.download(match.group(0))
            test=self.nemfile['DISPATCH_PRICE']
            print(test.loc[:,'PRICE':'RRP'])
            print(self.nemfile['DISPATCH_INTERCONNECTORRES'].loc[:,'INTERCONNECTORRES':'IMPORTLIMIT'])
            print(self.nemfile['DISPATCH_REGIONSUM'].loc[:,'REGIONSUM':'DISPATCHABLELOAD',])
        return (self.foundit)
        
    def download(self, link):
        """Dowloads nemweb zipfile from link into memory as a byteIO object.
        Returns a pandas object from the byteIO object """
